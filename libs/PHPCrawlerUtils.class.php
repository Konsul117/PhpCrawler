...
class PHPCrawlerUtils
{
    ...
    public static function buildURLFromLink($link, PHPCrawlerUrlPartsDescriptor $BaseUrlParts)
    {
        ...
        // convert hashbang URLs to crawlable versions with _escaped_fragment_
        if (stristr($link, '#!') !== false) {
            $parts = explode('#!', $link);
            $link = $parts[0].'?_escaped_fragment_='.urlencode($parts[1]);
        }
     ...
     * @return bool
     */
    public static function getContentEncodingFromHeader($header)
    {
